...
from sqlalchemy import Table, Column, Integer, String, Float, Boolean, Enum, ForeignKey
...
association_table = Table('association', Base.metadata,
                          ...
                          Column('playlist_id', String, ForeignKey('playlist.playlist_id', ondelete="CASCADE")))
...
class User(Base):
    ...
    playlists = relationship(
        "Playlist", secondary=association_table, back_populates="users", collection_class=attribute_mapped_collection('playlist_id'))
    data = relationship(
...
class Playlist(Base):
    ...
    owner = Column(String, ForeignKey(
        "user.username", ondelete="CASCADE"))
    candidate = Column(Boolean)
    users = relationship("User", secondary=association_table,
                         back_populates="playlists", collection_class=attribute_mapped_collection('username'))
